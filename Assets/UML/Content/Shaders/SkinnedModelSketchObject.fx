...
// SkinnedModel.fx
//
// Skinned model object space sketch shader
//
//-----------------------------------------------------------------------------
...
float4 PixelShader(PS_INPUT input) : COLOR0
{
	...
	//TEMP we've doubled the texture size of the hatch maps which doesn't look so good for object space mapping
...
//	input.TexCoord.y *= 0.5;
	    
	{
		if (bSketchInvert)
		{
			...
			float4 colora = tex2D(Sketch0Sampler, input.TexCoord);
			float4 colorb = tex2D(Sketch1Sampler, input.TexCoord);
			float4 colorc = tex2D(Sketch2Sampler, input.TexCoord);
			float4 colord = tex2D(Sketch3Sampler, input.TexCoord);
			float4 color1 = 0;

			color = (float4(1.0f-colora.r, 1.0f-colora.r, 1.0f-colora.b,1.0f)*a) +
