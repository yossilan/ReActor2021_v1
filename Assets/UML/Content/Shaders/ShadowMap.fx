...
DEPTH_VS_OUTPUT Depth_VS (VS_INPUT Input)
{
	...
	Output.Position = mul(float4(Input.Position.xyz, 1.0f), WorldCameraViewProjection);
...
{
	float depth = length(LightPosition0 - Input.WorldPosition) / FarClip;
...
//	Output.ShadowProjection = mul(Input.Position, WorldLightViewProjection0);
